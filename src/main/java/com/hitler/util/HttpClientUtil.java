...
//        CloseableHttpClient client = HttpClients.createDefault();
        ...
        PayLog.getLogger().info("请求地址："+url);
        PayLog.getLogger().info("请求参数："+nvps.toString());
        ...
        if(response.getStatusLine().getStatusCode()==302){  
            ...
            if(hs.length>0){   
               String rUrl = hs[0].toString().replaceAll("Location:", "").trim();
               //String resp =  send(rUrl, null, encoding);
               String resp = HttpUtil.Get(rUrl).getHtml();
               return resp;
